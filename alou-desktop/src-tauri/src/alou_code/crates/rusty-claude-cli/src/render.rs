...
impl Spinner {
    ...
    pub fn finish(
        ...
        self.frame_index = 0;
        // Print the finish label at the current cursor position (bottom of output).
        // We do NOT use RestorePosition here because the streaming output may have
        // moved the cursor far below the original spinner line. Restoring would
        // incorrectly place the "Done" message in the middle of the output.
        execute!(
            out,
            MoveToColumn(0),
            Clear(ClearType::CurrentLine),
...
impl MarkdownStreamState {
    /// Maximum pending content before forcing a flush to prevent memory exhaustion.
    const MAX_PENDING: usize = 16 * 1024 * 1024; // 16 MB

    #[must_use]
    pub fn push(&mut self, renderer: &TerminalRenderer, delta: &str) -> Option<String> {
        self.pending.push_str(delta);

        // Guard against unbounded pending growth: if pending exceeds MAX_PENDING,
        // force a flush even if we haven't reached a stream-safe boundary.
        if self.pending.len() > Self::MAX_PENDING {
            let pending = std::mem::take(&mut self.pending);
            return Some(renderer.markdown_to_ansi(&pending));
        }
...
fn normalize_nested_fences(markdown: &str) -> String {
    // Guard against pathological inputs that could cause capacity overflow.
    // RawVec::with_capacity panics when capacity * size_of::<T>() > isize::MAX.
    // For safety, skip fence normalization on inputs larger than 16 MB.
    if markdown.len() > 16 * 1024 * 1024 {
...
mod tests {
    ...
    fn renders_links_as_colored_markdown_labels() {
        let terminal_renderer = TerminalRenderer::new();
        let markdown_output =
            terminal_renderer.render_markdown("See [Alou](https://example.com/docs) now.");
        let plain_text = strip_ansi(&markdown_output);

        assert!(plain_text.contains("[Alou](https://example.com/docs)"));
        assert!(markdown_output.contains('\u{1b}'));
    }
