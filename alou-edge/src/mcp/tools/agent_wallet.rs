...
impl AgentWalletTool {
    ...
    async fn get_wallet(&self, session_id: &str, chain: &str) -> Result<Option<Value>> {
        ...
        match self.kv.get::<String>(&key).await? {
            Some(data) => {
                let wallet: Value = serde_json::from_str(&data).map_err(|e| {
                    AloudError::CacheError(format!("Failed to parse wallet data: {}", e))
                })?;
                Ok(Some(wallet))
            }
            None => Ok(None),
    ...
    async fn record_transaction(
        ...
        if let Some(data) = self.kv.get::<String>(&key).await? {
            let mut wallet: Value = serde_json::from_str(&data).map_err(|e| {
                AloudError::CacheError(format!("Failed to parse wallet data: {}", e))
            })?;

            // Add transaction to history
            if let Some(txs) = wallet
        ...
        match action {
            "create_wallet" => {
                let chain = args.get("chain").and_then(|v| v.as_str()).ok_or_else(|| {
                    AloudError::InvalidToolArgs("Missing 'chain' field".to_string())
                })?;
            ...
            "record_transaction" => {
                ...
                let transaction = args
                    .get("transaction")
                    .ok_or_else(|| {
                        AloudError::InvalidToolArgs("Missing 'transaction' field".to_string())
                    })?
                    .clone();

                self.record_transaction(session_id, chain, transaction)
                    .await?;
            ...
            "update_balance" => {
                ...
                let balance = args
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        AloudError::InvalidToolArgs("Missing 'balance' field".to_string())
                    })?;
