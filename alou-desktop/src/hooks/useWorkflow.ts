...
export interface WorkflowExecutionEvent {
  type: 'Started' | 'StepStarted' | 'StepCompleted' | 'StepFailed' | 'ProgressUpdated' | 'Completed' | 'Failed' | 'Paused' | 'Resumed' | 'Cancelled'
  execution_id: string
  workflow_id?: string
  step_id?: string
  progress?: number
...
export interface UseWorkflowConfig {
  ...
  apiKey?: string
  agentInfo?: AgentInfo
  onWorkflowMessage?: (message: string | WorkflowMessage) => void
}
...
export const useWorkflow = ({
  ...
  const [executionPolling, setExecutionPolling] = useState<Record<string, NodeJS.Timeout>>({})
  ...
  // 重试失败的步骤
  const retryStep = useCallback(async (workflowId: string, stepId: string) => {
  ...
  }, [clearPolling, checkWorkflowStatus])

  // 停止执行状态轮询
  const stopExecutionPolling = useCallback((executionId: string) => {
  ...
  useEffect(() => {
    ...
    const setupEventListener = async () => {
      try {
        unlisten = await listen<WorkflowExecutionEvent>('workflow-execution-event', (event) => {
            ...
            case 'ProgressUpdated':
              ...
              setExecutionProgress(prev => ({
                ...
                [executionEvent.workflow_id || 'unknown']: {
                  status: 'running',
                  progress: executionEvent.progress || 0,
