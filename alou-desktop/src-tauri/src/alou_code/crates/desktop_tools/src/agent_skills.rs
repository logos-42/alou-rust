//! Agent Skills Tool Adapter for alou_code Kernel
...
pub fn tool_spec() -> (
    ...
    let name = "desktop_agent_skills".to_string();
    let description = "Agent skills management and execution".to_string();
    let schema = json!({
        "type": "object",
        "properties": {
            "operation": {
                "type": "string",
                "enum": ["list", "get", "execute", "create", "delete"]
            },
            "skill": { "type": "string" },
            "args": { "type": "object" }
        },
