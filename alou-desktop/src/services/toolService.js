...
class ToolService {
  ...
  async executeLocalTool(toolId, args, timeout) {
    ...
    try {
      // 转换参数格式以匹配 Tauri 工具期望的格式
      const tauriArgs = this.convertArgsForTauri(toolId, args);
      
      const result = await invoke('execute_tool', {
        toolId,
        args: JSON.stringify(tauriArgs),
        timeout
      });
   ...
   */
  convertArgsForTauri(toolId, args) {
    switch (toolId) {
      case 'bash':
        // bash 工具期望 BashOperation::Execute 格式
        return {
          operation: 'Execute',
          shell: 'Bash',
      ...
      case 'skills':
        // skills 工具的参数转换
        return {
          skill_name: args.skill_name || args.name || '',
  ...
  async waitForToolCompletion(taskId, timeout = 60000) {
    ...
    const pollInterval = 1000; // 🔧 优化：减少到1秒轮询
