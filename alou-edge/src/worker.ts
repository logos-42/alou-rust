  ...
  async fetch(request: Request, env: any, ctx: any): Promise<Response> {
    ...
    if ((url.pathname === '/api/agent/chat' || url.pathname === '/api/claude-agent/query') && request.method === 'POST') {
      try {
        ...
        // 🚀 DEBUG: 强制使用 TypeScript SDK 进行测试
        console.log('[DEBUG] 强制使用 TypeScript SDK 路径进行测试');
        console.log('[DEBUG] 所有请求都走TypeScript SDK');
        ...
        console.log('[DEBUG] Session exists:', !!sessionStr);
        ...
        if (sessionStr) {
          ...
          metadata = session.agent_metadata;
        }

        // 强制使用TypeScript SDK
        if (true) { // 临时强制使用TypeScript SDK
              ...
              // 处理响应：如果是异步任务，返回任务ID
              if (result.task_id) {
                console.log(`[Worker] 返回异步任务ID: ${result.task_id}`);
