...
SERVER_IP="140.179.186.11"  # 你的云服务器IP
BACKEND_PORT=3001
...
server {
    listen 80;
    server_name SERVER_IP_PLACEHOLDER localhost;
    ...
    # 前端静态文件
    location / {
        root APP_DIR_PLACEHOLDER/frontend;
    ...
    # 健康检查
    location /health {
        proxy_pass http://127.0.0.1:BACKEND_PORT_PLACEHOLDER/api/health;
        access_log off;
    }
...
EOF
    ...
    # 替换占位符
    sudo sed -i "s/SERVER_IP_PLACEHOLDER/${SERVER_IP}/g" /etc/nginx/sites-available/${APP_NAME}
    sudo sed -i "s|APP_DIR_PLACEHOLDER|${APP_DIR}|g" /etc/nginx/sites-available/${APP_NAME}
    sudo sed -i "s/BACKEND_PORT_PLACEHOLDER/${BACKEND_PORT}/g" /etc/nginx/sites-available/${APP_NAME}
