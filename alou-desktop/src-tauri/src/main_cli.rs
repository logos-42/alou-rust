...
//!
//! 示例:
//!   alou-cli start        启动自主循环
//!   alou-cli status       查看状态
//!   alou-cli task add "测试"  添加任务
//!   alou-cli agent chat "你好"  和 Agent 对话
...
fn cmd_agent_chat(message: &str) {
    ...
    println!();
    println!("我收到你的消息: \"{}\"", message);
    println!();
    println!("自主循环已就绪，可以开始工作了。");
    println!("输入 alou-cli help 查看所有命令。");
}
...
fn cmd_help() {
    ...
    println!();
    println!("{}使用: alou-cli [命令] [参数]{}", YELLOW, RESET);
    println!();
    println!("{}自主循环控制:{}", CYAN, RESET);
    println!("  start           启动自主循环");
    println!("  stop            停止自主循环");
    println!("  pause           暂停");
    println!("  resume          恢复");
    println!("  status          查看状态");
    ...
    println!("  alou-cli start");
    println!("  alou-cli task add \"检查邮件\" \"检查未读邮件\" high");
    println!("  alou-cli agent chat \"你好\"");
    println!("  alou-cli status");
}
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "task" => {
            if args.len() < 3 {
                log_error("缺少任务操作");
                println!("用法: alou-cli task [add|list]");
                return;
            }
            match args[2].as_str() {
                "add" => {
                ...
                "list" => cmd_task_list(),
                _ => {
                    log_error(&format!("未知任务操作: {}", args[2]));
                }
            }
        }
        ...
        "agent" => {
            if args.len() < 3 {
                log_error("请提供对话内容");
                println!("用法: alou-cli agent chat <消息>");
                return;
            }
            match args[2].as_str() {
                "chat" => {
                ...
                _ => {
                    log_error(&format!("未知 Agent 操作: {}", args[2]));
                }
            }
        }
        ...
        _ => {
            ...
            println!("运行 {}alou-cli help{} 查看命令", GREEN, RESET);
        }
    }
}
