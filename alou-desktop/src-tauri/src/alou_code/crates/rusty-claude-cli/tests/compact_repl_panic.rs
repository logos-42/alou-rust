...
fn run_claw_repl(
    ...
    child.wait_with_output().expect("alou should finish")
}

fn python_pty_command(alou: &str) -> Command {
...
child = subprocess.Popen([alou], stdin=slave, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
os.close(slave)
