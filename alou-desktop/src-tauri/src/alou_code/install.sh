...
# builds the `alou` binary from the `rust/` workspace, and runs a
...
#   ALOU_BUILD_PROFILE=debug|release   same as --release toggle
#   ALOU_SKIP_VERIFY=1                 same as --no-verify
...
Environment overrides:
  ALOU_BUILD_PROFILE   debug | release
  ALOU_SKIP_VERIFY     set to 1 to skip verification
EOF
}
...
BUILD_PROFILE="${ALOU_BUILD_PROFILE:-debug}"
SKIP_VERIFY="${ALOU_SKIP_VERIFY:-0}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'alou' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/alou
...
step "Building the alou workspace (${BUILD_PROFILE})"
...
CLAW_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/alou"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    warn "verification skipped (--no-verify or ALOU_SKIP_VERIFY=1)"
else
    info "running: alou --version"
    if VERSION_OUT="$("${CLAW_BIN}" --version 2>&1)"; then
        ok "alou --version -> ${VERSION_OUT}"
    else
        error "alou --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: alou --help (smoke test)"
    if "${CLAW_BIN}" --help >/dev/null 2>&1; then
        ok "alou --help responded"
    else
        error "alou --help failed"
        exit 1
    fi
fi
...
cat <<EOF
${COLOR_GREEN}Alou Code is built and ready.${COLOR_RESET}
