...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Alou Code plugins",
        argument_hint: Some(
...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectAlou | Self::ProjectCodex | Self::ProjectClaude => {
                DefinitionScope::Project
            }
            Self::UserAlouConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserAlou | Self::UserCodex | Self::UserClaude => DefinitionScope::UserHome,
        }
    }
    ...
    match normalize_optional_args(args) {
        None | Some("list") => {
            ...
            match loader.load() {
                ...
                Err(err) => {
                    ...
                    Ok(format!(
                        "Config load error\n  Status           fail\n  Summary          runtime config failed to load; reporting partial MCP view\n  Details          {err}\n  Hint             `alou doctor` classifies config parse errors; fix the listed field and rerun\n\n{}",
        ...
        Some(args) if args.split_whitespace().next() == Some("list") && args.contains(' ') => {
            ...
            Ok(render_mcp_unsupported_action_text(
                args,
                "list accepts no filter argument; use `alou mcp list`",
            ))
        }
        Some(args) if matches!(args.split_whitespace().next(), Some("info" | "describe")) => {
            Ok(render_mcp_unsupported_action_text(
                args,
                "use `alou mcp show <server>` to inspect a server",
            ))
        }
...
fn render_mcp_unsupported_action_json(action: &str, hint: &str) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "alou mcp [list|show <server>|help]",
        },
    })
}
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectAlou,
            ancestor.join(".alou").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(alou_config_home) = env::var("ALOU_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserAlouConfigHome,
            PathBuf::from(alou_config_home).join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserAlou,
            home.join(".alou").join(leaf),
        );
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectAlou,
            ancestor.join(".alou").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectAlou,
            ancestor.join(".alou").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(alou_config_home) = env::var("ALOU_CONFIG_HOME") {
        let alou_config_home = PathBuf::from(alou_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserAlouConfigHome,
            alou_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserAlouConfigHome,
            alou_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(alou_config_home) = env::var("ALOU_CONFIG_HOME") {
        return Ok(PathBuf::from(alou_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".alou").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set ALOU_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       alou agents".to_string(),
        "  Sources          .alou/agents, ~/.alou/agents, $ALOU_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "alou agents [list|help]",
            "sources": [".alou/agents", "~/.alou/agents", "$ALOU_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       alou skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $ALOU_CONFIG_HOME/skills or ~/.alou/skills".to_string(),
        "  Sources          .alou/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.alou/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "alou skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$ALOU_CONFIG_HOME/skills or ~/.alou/skills",
            "sources": [
                ".alou/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       alou mcp [list|show <server>|help]".to_string(),
        "  Sources          .alou/settings.json, .alou/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            "slash_command": "/mcp [list|show <server>|help]",
            "direct_cli": "alou mcp [list|show <server>|help]",
            "sources": [".alou/settings.json", ".alou/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectAlou
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_alou",
        DefinitionSource::UserAlouConfigHome | DefinitionSource::UserCodexHome => {
            "user_alou_config_home"
        }
        DefinitionSource::UserAlou | DefinitionSource::UserCodex | DefinitionSource::UserClaude => {
            "user_alou"
        }
    }
}
...
mod tests {
    ...
    fn skills_show_and_list_filter_do_not_invoke_model() {
        ...
        // Regression for: `alou skills show plan` unexpectedly spawned a model session.
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Alou Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".alou").join("skills");
        let legacy_commands = workspace.join(".alou").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_alou");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_alou");
        ...
        assert_eq!(
            ...
            "alou skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       alou agents"));
        assert!(agents_help
            .contains("Sources          .alou/agents, ~/.alou/agents, $ALOU_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $ALOU_CONFIG_HOME/skills or ~/.alou/skills"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       alou mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".alou")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".alou").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".alou").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".alou/settings.json");
    ...
    fn mcp_degrades_gracefully_on_malformed_mcp_config_144() {
        // #144: mirror of #143's partial-success contract for `alou mcp`.
        ...
        fs::create_dir_all(workspace.join(".alou")).expect("create workspace .alou dir");
        fs::create_dir_all(&config_home).expect("create config home");
        // One valid server + one malformed entry missing `command`.
        fs::write(
            workspace.join(".alou.json"),
            r#"{
  "mcpServers": {
...
"#,
        ...
        .expect("write malformed .alou.json");
