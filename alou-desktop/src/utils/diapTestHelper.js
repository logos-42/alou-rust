...
class DiapTestHelper {
  ...
  async testIpfsStatus() {
    ...
    try {
      ...
      // 测试API连接
      const apiTest = await invoke('test_ipfs_api', { 
      ...
      // 获取节点信息
      if (apiTest.success) {
        const nodeInfo = await invoke('get_ipfs_info')
      ...
      return {
        daemon: daemonStatus,
        api: apiTest,
        ready: daemonStatus.process_running && apiTest.success
      }
    } catch (error) {
   ...
   * 测试完整的DIAP身份创建流程
   */
  async testFullDiapCreationFlow(sessionId, params = {}) {
    ...
    try {
      ...
      if (hasDiapIdentity(sessionId)) {
        ...
        return { success: true, identity: existing, isExisting: true }
      }
      ...
      const accessibilityTest = await invoke('test_ipns_on_public_gateway', {
  ...
  async testLocalStorage() {
    ...
    try {
      ...
      if (!retrieved || retrieved.did !== testIdentity.did) {
   ...
   * 运行所有测试
   */
  async runAllTests(sessionId = null) {
...
// 导出便捷函数
export const testDiapCreation = (sessionId, params) => 
  diapTestHelper.testFullDiapCreationFlow(sessionId, params)

export const testDiapStorage = () => 
  diapTestHelper.testLocalStorage()

export const testDiapIpfs = () => 
  diapTestHelper.testIpfsStatus()

export const runAllDiapTests = (sessionId) => 
