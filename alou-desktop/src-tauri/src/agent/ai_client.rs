...
impl AiClient {
    ...
    pub fn new(config: &super::config::UserApiConfig) -> Result<Self> {
        let provider: Box<dyn AiProvider> = match config.provider.to_lowercase().as_str() {
            ...
            // AlouCode Provider - 使用 DeepSeek 或 Anthropic API
            "aloucode" | "alou-code" => {
                let model = config.model.clone().unwrap_or_else(|| "deepseek-chat".to_string());
                if model.starts_with("claude") || model.starts_with("opus") || model.starts_with("sonnet") || model.starts_with("haiku") {
                    Box::new(providers::AlouCodeProvider::with_anthropic(
                        config.api_key.clone(),
                        config.model.clone(),
                    ))
                } else {
                    Box::new(providers::AlouCodeProvider::with_deepseek(
                        config.api_key.clone(),
                        config.model.clone(),
                    ))
                }
