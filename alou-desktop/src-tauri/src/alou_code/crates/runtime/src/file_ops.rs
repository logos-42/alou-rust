...
/// Maximum total output size for grep search results (1 MB).
const MAX_GREP_OUTPUT_SIZE: usize = 1 * 1024 * 1024;

/// Maximum display length for a single matched line (10 KB).
/// Lines exceeding this will be truncated with a marker.
const MAX_LINE_DISPLAY_LENGTH: usize = 10 * 1024;
...
pub fn grep_search(input: &GrepSearchInput) -> io::Result<GrepSearchOutput> {
        ...
        if output_mode == "content" {
            for index in matched_lines {
                ...
                for (current, line) in lines.iter().enumerate().take(end).skip(start) {
                    ...
                    let mut full_line = format!("{prefix}{line}");
                    if full_line.len() > MAX_LINE_DISPLAY_LENGTH {
                        full_line.truncate(MAX_LINE_DISPLAY_LENGTH);
                        full_line.push_str("... [truncated]");
                    }
                    if content_lines
                        .iter()
                        .map(|l: &String| l.len())
                        ...
                        > MAX_GREP_OUTPUT_SIZE
                    {
                        return Ok(GrepSearchOutput {
                            ...
                            num_matches: None,
                            applied_limit: input.head_limit,
                            applied_offset: input.offset,
                            truncated: Some(true),
                        });
                    }
    ...
    let content_output = if output_mode == "content" {
        ...
        let total_size = lines.iter().map(|l: &String| l.len()).sum::<usize>();
        let truncated = total_size >= MAX_GREP_OUTPUT_SIZE;
        return Ok(GrepSearchOutput {
            mode: Some(output_mode),
            num_files: filenames.len(),
            filenames,
...
mod tests {
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("aloud-native-{name}-{unique}"))
    }
