...
```bash
cd rust
./target/debug/alou doctor --output-format json
```
...
Set up a new repository with `.alou` config, `.alou.json`, `.gitignore` entries, and a `CLAUDE.md` guidance file:

```bash
cd /path/to/your/repo
./target/debug/alou init
```
...
```bash
./target/debug/alou init --output-format json
```
...
The `alou state` command reads `.alou/worker-state.json`, which is written by the interactive REPL or a one-shot prompt when a worker executes a task. This file contains the worker ID, session reference, model, and permission mode.

Prerequisite: You must run `alou` (interactive REPL) or `alou prompt <text>` at least once in the repository to produce the worker state file.

```bash
cd rust
./target/debug/alou state
```

JSON mode:
```bash
./target/debug/alou state --output-format json
```

If you run `alou state` before any worker has executed, you will see a helpful error:
```
error: no worker state file found at .alou/worker-state.json
  Hint: worker state is written by the interactive REPL or a non-interactive prompt.
  Run:   alou               # start the REPL (writes state on first turn)
  Or:    alou prompt <text> # run one non-interactive turn
  Then rerun: alou state [--output-format json]
```

## Advanced slash commands (Interactive REPL only)

These commands are available inside the interactive REPL (`alou` with no args). They extend the assistant with workspace analysis, planning, and navigation features.
