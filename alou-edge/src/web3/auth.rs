...
use crate::utils::crypto::{generate_nonce, ChainType};
...
impl WalletAuth {
    ...
    pub async fn verify_and_create_token(
        ...
        let auth_nonce =
            auth_nonce.ok_or_else(|| AloudError::AuthError("Nonce not found".to_string()))?;
        ...
        if !message.contains(&auth_nonce.nonce) {
            return Err(AloudError::AuthError(
                "Message does not contain nonce".to_string(),
            ));
        }
    ...
    pub fn verify_token(&self, token: &str) -> Result<WalletClaims> {
        let claims = self
            .jwt_key
            .verify_token::<WalletClaims>(token, None)
