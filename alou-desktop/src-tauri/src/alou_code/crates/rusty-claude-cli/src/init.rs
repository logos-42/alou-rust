...
const STARTER_ALOU_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Alou Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [".alou/settings.local.json", ".alou/sessions/", ".alouhip/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let alou_dir = cwd.join(".alou");
    artifacts.push(InitArtifact {
        name: ".alou/",
        status: ensure_dir(&alou_dir)?,
    });

    let alou_json = cwd.join(".alou.json");
    artifacts.push(InitArtifact {
        name: ".alou.json",
        status: write_file_if_missing(&alou_json, STARTER_ALOU_JSON)?,
    });
    ...
    let alou_md = cwd.join("ALOU.md");
    let content = render_init_claude_md(cwd);
    artifacts.push(InitArtifact {
        name: "ALOU.md",
        status: write_file_if_missing(&alou_md, &content)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.alou.json`; reserve `.alou/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `ALOU.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".alou/"));
        assert!(rendered.contains(".alou.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("ALOU.md        created"));
        assert!(root.join(".alou").is_dir());
        assert!(root.join(".alou.json").is_file());
        assert!(root.join("ALOU.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".alou.json")).expect("read alou json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".alou/settings.local.json"));
        assert!(gitignore.contains(".alou/sessions/"));
        assert!(gitignore.contains(".alouhip/"));
        let alou_md = fs::read_to_string(root.join("ALOU.md")).expect("read alou md");
        assert!(alou_md.contains("Languages: Rust."));
        assert!(alou_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("ALOU.md"), "custom guidance\n").expect("write existing alou md");
        fs::write(root.join(".gitignore"), ".alou/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("ALOU.md        skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".alou/"));
        assert!(second_rendered.contains(".alou.json"));
        assert!(second_rendered.contains("skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("ALOU.md        skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("ALOU.md")).expect("read existing alou md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".alou/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".alou/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".alouhip/").count(), 1);
