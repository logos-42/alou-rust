...
fn export_help_emits_bounded_json_when_requested_384() {
    ...
    assert_eq!(
        parsed["usage"],
        "alou export [--session <id|latest>] [--output <path>] [--output-format <format>]"
    );
...
fn export_help_preserves_plaintext_in_text_mode_384() {
    ...
    assert!(stdout.contains("Usage            alou export"));
...
fn doctor_and_resume_status_emit_json_when_requested() {
    ...
    assert_eq!(
        ...
        "https://github.com/ultraworkers/alou-code"
    );
    assert_eq!(
        install_source["deprecated_install"],
        "cargo install alou-code"
    );
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("alou-code");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "alou-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
