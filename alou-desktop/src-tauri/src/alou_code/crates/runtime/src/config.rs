...
pub const ALOU_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Alou runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".alou.json"),
            |parent| parent.join(".alou.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".alou.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".alou").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".alou").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("ALOU_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".alou")))
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".alou.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, ALOU_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".alou");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".alou");
        fs::create_dir_all(cwd.join(".alou")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".alou.json"),
        ...
        assert_eq!(ALOU_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".alou");
        let project_settings = cwd.join(".alou").join("settings.json");
        fs::create_dir_all(cwd.join(".alou")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
