...
class AgentCreationDiagnosticTool {
   ...
   * 诊断智能体创建过程中的问题
   */
  async diagnoseAgentCreation(sessionId) {
    ...
    try {
      ...
      this.diagnosticResults.errors.push(error.message)
   ...
   * 检查DIAP身份创建状态
   */
  async checkDiapIdentityCreation(sessionId) {
    ...
    try {
      ...
      this.diagnosticResults.errors.push(`DIAP身份检查失败: ${error.message}`)
    }
  }
   ...
   * 检查头像状态
   */
  async checkAvatarStatus(sessionId) {
    console.log('🔍 检查头像状态...')
    ...
    try {
      ...
      this.diagnosticResults.errors.push(`头像状态检查失败: ${error.message}`)
    }
  }
   ...
   * 检查数据存储状态
   */
  async checkDataStorage(sessionId) {
    ...
    try {
      ...
      this.diagnosticResults.errors.push(`数据存储检查失败: ${error.message}`)
    }
  }
   ...
   * 生成诊断报告
   */
  generateDiagnosticReport(sessionId) {
    const report = {
      sessionId,
      timestamp: Date.now(),
...
if (typeof window !== 'undefined') {
  window.AlouAgentDiagnostic = {
    diagnoseAgentCreation: (sessionId) => agentCreationDiagnosticTool.diagnoseAgentCreation(sessionId),
    checkDiapIdentity: (sessionId) => agentCreationDiagnosticTool.checkDiapIdentityCreation(sessionId),
    checkAvatar: (sessionId) => agentCreationDiagnosticTool.checkAvatarStatus(sessionId),
