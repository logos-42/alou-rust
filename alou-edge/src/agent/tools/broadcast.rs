...
impl BroadcastTool {
    ...
    pub async fn broadcast_eth_transaction(
        &self,
        signed_tx: &str,
        ...
        if let Some(tx_hash) = response.get("result").and_then(|r| r.as_str()) {
            ...
            Err(AloudError::AgentError(format!(
                "Broadcast error: {}",
                error
            )))
        } else {
            Err(AloudError::AgentError(
                "Failed to broadcast transaction".to_string(),
            ))
        }
    }
    ...
    pub async fn get_eth_transaction_receipt(
        &self,
        tx_hash: &str,
        chain_hint: Option<&str>,
    ...
    pub async fn is_transaction_confirmed(&self, tx_hash: &str, chain: &str) -> Result<bool> {
        match chain.to_lowercase().as_str() {
            "eth" | "ethereum" => {
                match self.get_eth_transaction_receipt(tx_hash, Some(chain)).await {
                    Ok(receipt) => Ok(receipt.block_number.is_some()),
                    Err(_) => Ok(false),
                }
            }
            "sol" | "solana" => match self.get_sol_transaction_status(tx_hash).await {
...
impl McpTool for BroadcastTool {
    ...
    async fn execute(&self, args: Value, context: &AgentContext) -> Result<Value> {
        ...
        match chain {
            "eth" => {
                let tx_hash = self
                    .broadcast_eth_transaction(signed_tx, chain_hint)
                    .await?;
                let summary = format!("交易已广播，等待确认：{}", tx_hash);
                let tracker = json!({
