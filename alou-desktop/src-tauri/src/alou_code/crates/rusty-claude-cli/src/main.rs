...
const DEFAULT_MODEL: &str = "minimax-2.7";
...
const OFFICIAL_REPO_URL: &str = "https://github.com/logos-42/alou-code";
const OFFICIAL_REPO_SLUG: &str = "ultraworkers/alou-code";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install alou-code";
...
fn main() {
    if let Err(error) = run() {
        ...
        } else if message.contains("`alou --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `alou --help` for usage."
            );
        }
        std::process::exit(1);
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `alou prompt --help`) should show
            ...
            "-p" => {
                // Alou Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Alou Code compat: --print makes output non-interactive
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`alou {command_name}` is a slash command. Use `alou --resume SESSION.jsonl /{command_name}` or start `alou` and run `/{command_name}`."
        )
    } else {
        format!(
            "`alou {command_name}` is a slash command. Start `alou` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`alou {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `alou` and run it there, or use `alou --resume SESSION.jsonl {command_name}` / `alou --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `alou --help` for CLI usage, or start `alou` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `alou` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
fn resolve_model_alias(model: &str) -> &str {
    match model {
        ...
        "minimax" | "minimax-2.7" => "MiniMax-M2.7",
        _ => model,
    }
}
...
/// Starts a minimal Model Context Protocol server that exposes alou's
...
/// Read `.alou/worker-state.json` from the current working directory and print it.
/// This is the file-based worker observability surface: `push_event()` in `worker_boot.rs`
/// atomically writes state transitions here so external observers (alouhip, orchestrators)
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".alou").join("worker-state.json");
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `alou login` is removed"
                .to_string(),
        ])
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `alou` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `alou dump-manifests --manifests-dir /path/to/upstream`.";
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .alou/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `alou` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: alou is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && alou",
            cwd.display()
        );
        ...
        let message = format!(
            "alou is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && alou",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
fn run_repl(
    ...
    // Start TUI render loop to show status bar, thinking indicator, etc.
    let render_handle = cli.start_tui_render_loop(Arc::clone(&cli.tui_app));
    ...
    loop {
        ...
        // Stop the TUI render loop before exiting
        render_handle.stop();
    }
...
struct LiveCli {
    ...
    tui_app: Arc<Mutex<crate::tui::TuiApp>>,
}
...
impl LiveCli {
    fn new(
        ...
        let tui_state = Arc::new(Mutex::new(crate::tui::TuiState::new()));
        let tui_app = Arc::new(Mutex::new(crate::tui::TuiApp::with_state(Arc::clone(
            &tui_state,
        ))));
        let cli = Self {
            model,
 ...
 ██╔══██║██║   ██║    ██║██║   ██║       \n\
 ██║  ██║██████╗███████║ ████████║       \n\
 ╚═╝  ╚═╝╚═════╝ ╚═════╝ ╚══════╝        \x1b[0m \x1b[38;5;226mcode\x1b[0m 🪼\n\n\
  \x1b[2mModel\x1b[0m            {}\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let reporter =
            crate::tui::streaming_reporter::TuiStreamingReporter::new(Arc::clone(&self.tui_state));
        if let Some(rt) = runtime.runtime.as_mut() {
            rt.api_client_mut()
                .set_streaming_reporter(Some(Arc::new(reporter) as _));
        }

        let render_handle = self.start_tui_render_loop(Arc::clone(&self.tui_app));
        {
            // Handle PoisonError gracefully: if a previous panic poisoned the mutex,
            // recover the inner value and continue. The data may be partially updated
            // but we can still set streaming/processing flags safely.
            let mut state = match self.tui_state.lock() {
                Ok(s) => s,
                Err(poisoned) => {
                    eprintln!(
                        "[WARN] tui_state mutex was poisoned (previous panic), recovering"
                    );
                    poisoned.into_inner()
                }
            };
        ...
        render_handle.stop();
        {
            let mut state = match self.tui_state.lock() {
                ...
                Err(poisoned) => {
                    eprintln!(
                        "[WARN] tui_state mutex was poisoned on cleanup, recovering"
                    );
                    poisoned.into_inner()
                }
            };
        ...
        // Handle context window blocked error with iterative auto-compact retry
        let turn_result: Result<runtime::TurnSummary, runtime::RuntimeError> =
        'compact_block: {
            match result {
                Ok(summary) => break 'compact_block Ok(summary),
                Err(ref error) if error.to_string().contains("context_window_blocked") => {
                    ...
                    loop {
                        iter += 1;
                        eprintln!(
                            "[auto-compact] Iteration {}: compacting session (context window blocked)...",
                            iter
                        );

                        let compact = runtime.compact(runtime::CompactionConfig::default());

                        if compact.removed_message_count == 0 {
                            eprintln!(
                                "[auto-compact] No more messages to compact ({} remain), giving up",
                                compact.compacted_session.messages.len()
                            );
                            break;
                        }

                        eprintln!(
                            "[auto-compact] Removed {} messages, applying and retrying...",
                            compact.removed_message_count
                        );

                        // Critical: update runtime's session with the compacted session
                        // otherwise we're compacting the same oversized session every iteration
                        *runtime.session_mut() = compact.compacted_session;

                        if iter >= max_iterations {
                            eprintln!(
                                "[auto-compact] Hit max iterations ({}), giving up",
                                max_iterations
                            );
                            break;
                        }
                        ...
                        match runtime.run_turn(input, Some(&mut permission_prompter)) {
                            Ok(summary) => {
                                eprintln!(
                                    "[auto-compact] Succeeded after {} iteration(s)",
                                    iter
                                );
                                break 'compact_block Ok(summary);
                            }
                            Err(ref retry_err)
                                if retry_err.to_string().contains("context_window_blocked") =>
                            {
                                last_error = retry_err.clone();
                            }
                            Err(ref retry_err) => {
                                last_error = retry_err.clone();
                                break;
        ...
        match turn_result {
            Ok(summary) => {
                ...
                let _ = write!(stdout, "{}", crate::tui::layout::ansi::clear_line());
                stdout.flush()?;
                println!();
    ...
    fn start_tui_render_loop(&self, app: Arc<Mutex<crate::tui::TuiApp>>) -> RenderHandle {
        let (stop_tx, stop_rx) = mpsc::channel();

        thread::spawn(move || {
            let mut stdout = io::stdout();
            let mut last_render = Instant::now();
            let render_interval = Duration::from_millis(16); // 60fps for smooth animation

            loop {
                if let Ok(stop) = stop_rx.recv_timeout(render_interval) {
                    if stop {
                        break;
                    }
                    continue;
    ...
    fn print_mcp(
        ...
        // `alou mcp serve` starts a stdio MCP server exposing alou's built-in
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .alou/sessions/<session-id>.jsonl".to_string(),
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .alou/sessions/. Extract the stem (drop the .jsonl extension).
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Related          /status · alou --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            alou sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · alou status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            alou doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · alou --resume latest /doctor"
            .to_string(),
    }
}
...
fn build_runtime(
    ...
    streaming_reporter: Option<
        std::sync::Arc<dyn crate::tui::streaming_reporter::StreamingEventReporter>,
    >,
...
struct AnthropicRuntimeClient {
    ...
    progress_reporter: Option<InternalPromptProgressReporter>,
    streaming_reporter:
        Option<std::sync::Arc<dyn crate::tui::streaming_reporter::StreamingEventReporter>>,
    reasoning_effort: Option<String>,
}
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // alou at its fake Anthropic endpoint. We also attach a
        ...
        let client = match detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::Xai | ProviderKind::OpenAi | ProviderKind::DeepSeek => {
                ...
                // OpenRouter, xAI, DashScope, Ollama, DeepSeek, and any other
    ...
    fn set_streaming_reporter(
        &mut self,
        reporter: Option<
            std::sync::Arc<dyn crate::tui::streaming_reporter::StreamingEventReporter>,
        >,
    ) {
        self.streaming_reporter = reporter;
    }
}
...
impl ApiClient for AnthropicRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let resolved_model = resolve_model_alias_with_config(&self.model);
        ...
        loop {
            ...
            match event {
                ...
                ApiStreamEvent::ContentBlockStart(start) => {
                    let block_type = match &start.content_block {
                        OutputContentBlock::Text { .. } => "text",
                        OutputContentBlock::ToolUse { .. } => "tool_use",
                        OutputContentBlock::Thinking { .. } => "thinking",
                        OutputContentBlock::RedactedThinking { .. } => "redacted_thinking",
                    };
                    if let Some(reporter) = &self.streaming_reporter {
                        reporter.on_content_block_start(block_type);
                    }
                    push_output_block(
                        start.content_block,
                        out,
                ...
                ApiStreamEvent::MessageDelta(delta) => {
                    if let Some(reporter) = &self.streaming_reporter {
                        reporter.on_usage_update(delta.usage.token_usage());
                    }
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   alou --resume {session_id} /compact"
    ));
...
fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    ...
    match name {
        ...
        "PolymarketGetOrderbook" => format_polymarket_orderbook_result(icon, &parsed),
...
fn format_polymarket_orderbook_result(icon: &str, parsed: &serde_json::Value) -> String {
    let bids_count = parsed
        .get("bids")
        .and_then(|v| v.as_array())
        .map(|arr| arr.len())
        .unwrap_or(0);
    let asks_count = parsed
        .get("asks")
        .and_then(|v| v.as_array())
        .map(|arr| arr.len())
        .unwrap_or(0);
    ...
    let rendered_output =
        serde_json::to_string_pretty(parsed).unwrap_or_else(|_| parsed.to_string());
    let total_chars = rendered_output.chars().count();

    let summary = format!(
        "▶ \x1b[1;36mOrderbook\x1b[0m {icon} {} bids, {} asks, spread {} ({} chars total)",
        bids_count, asks_count, spread, total_chars
    );
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "alou v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  alou [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  alou [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  alou [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  alou --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  alou help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  alou version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  alou status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  alou sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  alou doctor")?;
    writeln!(
        out,
    ...
    writeln!(out, "  alou dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  alou bootstrap-plan")?;
    writeln!(out, "  alou agents")?;
    writeln!(out, "  alou mcp")?;
    writeln!(out, "  alou skills")?;
    writeln!(out, "  alou system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  alou init")?;
    writeln!(
        out,
        "  alou export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .alou/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  alou --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  alou --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  alou --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  alou --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  alou --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  alou --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  alou agents")?;
    writeln!(out, "  alou mcp show my-server")?;
    writeln!(out, "  alou /skills")?;
    writeln!(out, "  alou doctor")?;
    ...
    writeln!(out, "  alou init")?;
    writeln!(out, "  alou export")?;
    writeln!(out, "  alou export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   alou --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".alou")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".alou").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("ALOU_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("RUSTY_CLAUDE_PERMISSION_MODE").ok();
        std::env::set_var("ALOU_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");

        let resolved = with_current_dir(&cwd, super::default_permission_mode);

        match original_config_home {
            Some(value) => std::env::set_var("ALOU_CONFIG_HOME", value),
            None => std::env::remove_var("ALOU_CONFIG_HOME"),
        }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("alou --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("alou --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .alou/sessions/<session-id>.jsonl"));
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("ALOU_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("alou help"));
        assert!(help.contains("alou version"));
        assert!(help.contains("alou status"));
        assert!(help.contains("alou sandbox"));
        assert!(help.contains("alou init"));
        assert!(help.contains("alou agents"));
        assert!(help.contains("alou mcp"));
        assert!(help.contains("alou skills"));
        assert!(help.contains("alou /skills"));
        assert!(help.contains("ultraworkers/alou-code"));
        assert!(help.contains("cargo install alou-code"));
        assert!(!help.contains("alou login"));
        assert!(!help.contains("alou logout"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("alou --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("alou --resume latest"));
        assert!(help.contains("alou --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".alou/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".alou/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".alou/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("alou-cli-{label}-{nanos}"))
    }
    ...
    fn init_template_mentions_detected_rust_workspace() {
        ...
        assert!(rendered.contains("# ALOU.md"));
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_uses_explicit_manifest_dir() {
        let root = std::env::temp_dir().join(format!(
            "alou_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
