...
/// Read the `.alou-base` file from the given directory and return the trimmed
/// commit hash, or `None` when the file is absent or empty.
pub fn read_alou_base_file(cwd: &Path) -> Option<String> {
    let path = cwd.join(".alou-base");
...
/// fall back to reading `.alou-base` from `cwd`.
pub fn resolve_expected_base(flag_value: Option<&str>, cwd: &Path) -> Option<BaseCommitSource> {
    ...
    read_alou_base_file(cwd).map(BaseCommitSource::File)
}
...
mod tests {
    ...
    fn reads_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".alou-base"), "abc1234def5678\n").expect("write .alou-base");

        // when
        let value = read_alou_base_file(&root);
    ...
    fn returns_none_for_empty_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".alou-base"), "  \n").expect("write empty .alou-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".alou-base"), "from_file\n").expect("write .alou-base");
    ...
    fn matches_with_claw_base_file_in_real_repo() {
        ...
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".alou-base"), format!("{sha}\n")).expect("write .alou-base");
        let source = resolve_expected_base(None, &root);
    ...
    fn diverged_with_claw_base_file_after_new_commit() {
        ...
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".alou-base"), format!("{old_sha}\n")).expect("write .alou-base");
