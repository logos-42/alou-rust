...
/// Write current worker state to `.alou/worker-state.json` under the worker's cwd.
/// This is the file-based observability surface: external observers (alouhip, orchestrators)
...
fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".alou");
...
mod tests {
    ...
    fn startup_timeout_classifies_tool_permission_prompt() {
        ...
        let timed_out = registry
            .observe_startup_timeout(&worker.worker_id, "alou prompt", true, true)
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".alou").join("worker-state.json");
        assert!(
            state_path.exists(),
