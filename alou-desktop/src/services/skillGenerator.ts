...
interface SkillDefinition {
  ...
  description: string;
  version: string;
  category: 'automation' | 'development' | 'research' | 'web3' | 'other';
...
请根据用户输入生成符合上述结构的技能定义，只返回 JSON 格式，不要包含其他文字。`;
   ...
   * @returns 完整提示词
   */
  private buildPrompt(description: string, agentInfo: AgentInfo, context: SkillContext): string {
...
5. 选择合适的类别`;
   ...
   * @returns 解析后的技能定义
   */
  private parseSkillDefinition(content: any): SkillDefinition {
    try {
      // 尝试直接解析 JSON
   ...
   * @returns 验证结果
   */
  validateSkillDefinition(skillDefinition: any): ValidationResult {
    const errors: string[] = [];
    ...
    if (skillDefinition.actions) {
      Object.entries(skillDefinition.actions).forEach(([actionName, actionDef]: [string, any]) => {
...
请分析示例的模式，生成一个能够实现目标功能的完整技能定义。`;
  ...
  /**
   * 优化现有技能
   * @param currentSkill - 当前技能定义
   * @param improvementGoal - 改进目标
   * @returns 优化后的技能定义
   */
  async optimizeSkill(
    currentSkill: SkillDefinition, 
    improvementGoal: string
  ): Promise<OptimizeSkillResult> {
    try {
...
4. 优化用户体验`;
   ...
   * 识别改进点
   * @param original - 原始技能
   * @param optimized - 优化后技能
   * @returns 改进点列表
   */
  private identifyImprovements(original: SkillDefinition, optimized: SkillDefinition): SkillImprovement[] {
    const improvements: SkillImprovement[] = [];
    
    // 比较字段变化
    Object.keys(optimized).forEach(key => {
      if (JSON.stringify((original as any)[key]) !== JSON.stringify((optimized as any)[key])) {
        improvements.push({
          field: key,
          original: (original as any)[key],
          optimized: (optimized as any)[key],
          type: 'modified'
        });
      }
    });
