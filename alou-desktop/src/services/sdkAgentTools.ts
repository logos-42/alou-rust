 ...
 * LSP工具配置
 */
export const LSP_TOOLS: Record<string, ToolDefinition> = {
  CODE_COMPLETION: {
 ...
 * Spec工具配置
 */
export const SPEC_TOOLS: Record<string, ToolDefinition> = {
  CREATE_SPEC: {
    name: "create_spec",
    description: "创建规格文档",
 ...
 * 所有SDK工具
 */
export const SDK_AGENT_TOOLS: Record<string, ToolDefinition> = {
  ...LSP_TOOLS,
  ...SPEC_TOOLS
}
 ...
 * SDK工具类别
 */
export const SDK_TOOL_CATEGORIES: Record<string, string[]> = {
  DEVELOPMENT: [
    "code_completion",
    "code_diagnostics", 
    "code_format",
 ...
 * 执行LSP工具
 */
export async function executeLspTool(toolName: string, parameters: Record<string, any>): Promise<any> {
  try {
    switch (toolName) {
 ...
 * 执行Spec工具
 */
export async function executeSpecTool(toolName: string, parameters: Record<string, any>): Promise<any> {
  try {
    switch (toolName) {
      case 'create_spec':
 ...
 * 执行SDK工具
 */
export async function executeSdkTool(toolName: string, parameters: Record<string, any>): Promise<any> {
  // 检查是LSP工具还是Spec工具
 ...
 * 获取所有可用的SDK工具
 */
export function getAllSdkTools(): ToolDefinition[] {
 ...
 * 获取指定类别的SDK工具
 */
export function getSdkToolsByCategory(category: string): ToolDefinition[] {
  const tools = SDK_TOOL_CATEGORIES[category.toUpperCase()] || []
  return tools.map(toolName => {
    const key = Object.keys(SDK_AGENT_TOOLS).find(k => SDK_AGENT_TOOLS[k].name === toolName)
    return key ? SDK_AGENT_TOOLS[key] : null
