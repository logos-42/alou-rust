...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            ...
            description: "List active prediction markets on Polymarket. Returns market condition IDs, questions, outcome prices, and active status.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "active_only": { "type": "boolean", "description": "Only return markets that are active and accepting orders" },
                    "limit": { "type": "integer", "minimum": 1, "maximum": 100, "description": "Maximum number of markets to return" }
                },
                "required": [],
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            ...
            description: "Get the order book (bids and asks) for a specific token on Polymarket.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "token_id": { "type": "string", "description": "The token ID to get the order book for" }
                },
                "required": ["token_id"],
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            ...
            description: "Check the connected Polymarket wallet balance (MATIC and USDC on Polygon).",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "required": [],
        ...
        ToolSpec {
            ...
            description: "Place a limit order on Polymarket. Requires POLY_API_KEY, POLY_API_SECRET, POLY_PASSPHRASE, and PRIVATE_KEY environment variables to be set.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "token_id": { "type": "string", "description": "The token ID to trade" },
                    "side": { "type": "string", "enum": ["BUY", "SELL"], "description": "Buy or sell" },
                    "price": { "type": "number", "minimum": 0.01, "maximum": 0.99, "description": "Limit price between 0.01 and 0.99" },
                    "size": { "type": "number", "minimum": 1.0, "description": "Order size in USDC" }
                },
        ...
        ToolSpec {
            ...
            description: "Get your open and recent orders from Polymarket CLOB. Requires PRIVATE_KEY environment variable (or POLYMARKET_PRIVATE_KEY).",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "market": { "type": "string", "description": "Filter by market condition ID (hex string, e.g. \"0x...\")" },
                    "asset_id": { "type": "string", "description": "Filter by asset/token ID (hex string)" },
                    "limit": { "type": "number", "description": "Maximum number of orders to return per page (default 50, max 200)" },
                    "next_cursor": { "type": "string", "description": "Cursor for pagination (from previous response next_cursor)" }
                },
                "additionalProperties": false
            }),
            required_permission: PermissionMode::ReadOnly,
        ...
        ToolSpec {
            ...
            description: "Get your trade history from Polymarket CLOB. Requires PRIVATE_KEY environment variable (or POLYMARKET_PRIVATE_KEY).",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "market": { "type": "string", "description": "Filter by market condition ID (hex string, e.g. \"0x...\")" },
                    "asset_id": { "type": "string", "description": "Filter by asset/token ID (hex string)" },
                    "taker_address": { "type": "string", "description": "Filter by taker address (hex string)" },
                    "maker_address": { "type": "string", "description": "Filter by maker address (hex string)" },
                    "before": { "type": "number", "description": "Filter trades before this Unix timestamp" },
                    "after": { "type": "number", "description": "Filter trades after this Unix timestamp" },
                    "limit": { "type": "number", "description": "Maximum number of trades to return per page (default 50, max 200)" },
                    "next_cursor": { "type": "string", "description": "Cursor for pagination (from previous response next_cursor)" }
                },
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            ...
            description: "Approve USDC and CTF tokens for Polymarket trading contracts. This enables trading on Polymarket CLOB. Requires PRIVATE_KEY environment variable.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        // ── Polymarket tools ──
        "PolymarketListMarkets" => {
            from_value::<PolymarketListMarketsInput>(input).and_then(run_polymarket_list_markets)
        }
        "PolymarketGetOrderbook" => {
            from_value::<PolymarketGetOrderbookInput>(input).and_then(run_polymarket_get_orderbook)
        }
        "PolymarketWalletBalance" => run_polymarket_wallet_balance(),
        "PolymarketPlaceOrder" => {
            maybe_enforce_permission_check(enforcer, "PolymarketPlaceOrder", input)?;
            from_value::<PolymarketPlaceOrderInput>(input).and_then(run_polymarket_place_order)
        }
        "PolymarketGetOrders" => {
            maybe_enforce_permission_check(enforcer, "PolymarketGetOrders", input)?;
            from_value::<PolymarketGetOrdersInput>(input).and_then(run_polymarket_get_orders)
        }
        "PolymarketGetTrades" => {
            maybe_enforce_permission_check(enforcer, "PolymarketGetTrades", input)?;
            from_value::<PolymarketGetTradesInput>(input).and_then(run_polymarket_get_trades)
        }
...
fn run_polymarket_list_markets(input: PolymarketListMarketsInput) -> Result<String, String> {
    to_pretty_json(execute_polymarket_list_markets(&input)?)
}

fn run_polymarket_get_orderbook(input: PolymarketGetOrderbookInput) -> Result<String, String> {
    to_pretty_json(execute_polymarket_get_orderbook(&input)?)
}

fn run_polymarket_wallet_balance() -> Result<String, String> {
    to_pretty_json(execute_polymarket_wallet_balance()?)
}

fn run_polymarket_place_order(input: PolymarketPlaceOrderInput) -> Result<String, String> {
    to_pretty_json(execute_polymarket_place_order(&input)?)
}

fn run_polymarket_get_orders(input: PolymarketGetOrdersInput) -> Result<String, String> {
    to_pretty_json(execute_polymarket_get_orders(&input)?)
}

fn run_polymarket_get_trades(input: PolymarketGetTradesInput) -> Result<String, String> {
    to_pretty_json(execute_polymarket_get_trades(&input)?)
}
...
// ── Polymarket re-exports from runtime ──
pub use runtime::polymarket::{
    execute_polymarket_get_orderbook, execute_polymarket_get_orders, execute_polymarket_get_trades,
    execute_polymarket_list_markets, execute_polymarket_place_order,
    execute_polymarket_wallet_balance, PolymarketGetOrderbookInput, PolymarketGetOrdersInput,
    PolymarketGetTradesInput, PolymarketListMarketsInput, PolymarketMarketSummary,
    PolymarketOrderItem, PolymarketOrderResult, PolymarketOrderbookEntry,
    PolymarketOrderbookResult, PolymarketOrdersResult, PolymarketOutcomeSummary,
    PolymarketPlaceOrderInput, PolymarketTradeItem, PolymarketTradesResult, PolymarketWalletResult,
};
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("aloud-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(claw_config_home) = std::env::var("ALOU_CONFIG_HOME") {
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.alou/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".alou"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".alou"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("aloud-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".aloud-agents"));
    }
    Ok(cwd.join(".aloud-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "clawguide" | "clawguideagent" | "guide" => String::from("alou-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".alou").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("ALOU_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use ALOU_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".alou"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("aloud-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .alou/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let claw_dir = worktree.join(".alou");
        fs::create_dir_all(&claw_dir).expect("create .alou dir");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .alou/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".alou").join("worker-state.json");
    ...
    fn stall_detect_and_restart_recovery_end_to_end() {
        // Worker stalls at trust_required, alouhip restarts instead of resolving
        let created = execute_tool(
            "WorkerCreate",
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".alou").join("skills").join("plan");
        let command_dir = root.join(".alou").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".alou/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".alou/commands/handoff.md"));
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".alou")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".alou")).expect("cwd dir");
        std::fs::write(
            home.join(".alou").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".alou")).expect("cwd dir");
        std::fs::write(
            cwd.join(".alou").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".alou").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".alou").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        let result = run_task_packet(TaskPacket {
            ...
            repo: "alou-code-parity".to_string(),
        ...
        assert_eq!(output["task_packet"]["repo"], "alou-code-parity");
        assert_eq!(
