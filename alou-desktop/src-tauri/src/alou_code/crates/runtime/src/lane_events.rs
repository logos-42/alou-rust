...
pub struct LaneOwnership {
    ...
    pub owner: String,
    /// Workflow scope (e.g., alou-code-dogfood, external-git-maintenance)
...
pub struct LaneEventMetadata {
    ...
    /// Emitter identity (e.g., aloud, plugin-name, operator-id)
    #[serde(skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn lane_ownership_binding_includes_workflow_scope() {
        let ownership = LaneOwnership {
            owner: "alou-1".to_string(),
            workflow_scope: "alou-code-dogfood".to_string(),
            watcher_action: WatcherAction::Act,
        };

        assert_eq!(ownership.owner, "alou-1");
        assert_eq!(ownership.workflow_scope, "alou-code-dogfood");
    ...
    fn lane_event_metadata_includes_us014_fields() {
        let meta = LaneEventMetadata::new(42, EventProvenance::LiveLane)
            .with_environment("production")
            .with_emitter("aloud-1")
            .with_confidence(ConfidenceLevel::High);

        assert_eq!(meta.environment_label, Some("production".to_string()));
        assert_eq!(meta.emitter_identity, Some("aloud-1".to_string()));
