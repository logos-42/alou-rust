...
class DesktopWalletService {
    ...
    this._initWalletConnectPromise = (async () => {
      try {
        ...
        const providerOptions = {
          ...
          metadata: {
            name: 'Alou Desktop',
            description: 'Alou Desktop - Web3 AI Agent',
  ...
  async verifySignature(address, message, signature) {
    try {
      ...
      console.warn('Ethers verification failed, trying backend:', error)
      
      // 备用方案：使用后端验证
      try {
        const isValid = await invoke('verify_wallet_signature', {
          address,
          message,
          signature,
          chain: 'ethereum',
