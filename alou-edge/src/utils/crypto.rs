...
pub fn verify_ethereum_signature(address: &str, message: &str, signature: &str) -> Result<bool> {
    ...
    let sig_bytes = hex::decode(signature).map_err(|_| AloudError::InvalidSignature)?;
    ...
    let signature =
        K256Signature::from_bytes(&sig_data.into()).map_err(|_| AloudError::InvalidSignature)?;

    // Recover public key from signature
    let recovered_key = VerifyingKey::recover_from_prehash(
        &message_hash,
        &signature,
        k256::ecdsa::RecoveryId::try_from(recovery_id).map_err(|_| AloudError::InvalidSignature)?,
    )
...
mod tests {
    ...
    fn test_chain_type_from_str() {
        assert_eq!(
            ChainType::from_str("ethereum").unwrap(),
            ChainType::Ethereum
        );
