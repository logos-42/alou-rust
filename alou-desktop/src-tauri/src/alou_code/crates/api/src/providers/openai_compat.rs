...
const DEEPSEEK_ENV_VARS: &[&str] = &["ANTHROPIC_API_KEY"];
...
impl OpenAiCompatConfig {
    ...
    /// Uses ANTHROPIC_API_KEY and ANTHROPIC_BASE_URL.
    #[must_use]
    pub const fn deepseek() -> Self {
        Self {
            ...
            max_request_body_bytes: 100_000_000, // 100MB, same as OpenAI
        }
    }

    /// MiniMax OpenAI-compatible endpoint.
    /// Uses MINIMAX_API_KEY and MINIMAX_BASE_URL.
    #[must_use]
...
impl OpenAiCompatClient {
    ...
    /// Create client using a specific env var for API key (e.g., "ANTHROPIC_API_KEY" for DeepSeek)
    pub fn from_env_with_auth(
        config: OpenAiCompatConfig,
        api_key_env: &'static str,
    ) -> Result<Self, ApiError> {
        let Some(api_key) = read_env_non_empty(api_key_env)? else {
            let env_vars: &'static [&'static str] = Box::leak(vec![api_key_env].into_boxed_slice());
            return Err(ApiError::missing_credentials(
                config.provider_name,
                env_vars,
            ));
