...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  alou --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    let config_home = temp_dir.join("home").join(".alou");
    fs::create_dir_all(project_dir.join(".alou")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".alou").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = run_claw_with_env(
        ...
        &[("ALOU_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated ALOU_CONFIG_HOME so ~/.claw/settings.json is not loaded,
...
fn run_claw_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    ...
    command.output().expect("alou should launch")
}
