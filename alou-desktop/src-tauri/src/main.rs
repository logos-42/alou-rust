...
// alou-code 集成桥接层
mod alou_bridge;
use crate::alou_bridge::{
    AlouBridge, SessionManager, UnifiedSession,
    create_unified_session, get_unified_session, send_alou_message,
    execute_alou_tool, list_alou_tool_executors, health_check_alou_bridge,
};
...
fn main() {
    tauri::Builder::default()
        ...
        // alou-code Bridge 状态管理
        .manage(std::sync::Arc::new(AlouBridge::new()))
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // alou-code Bridge commands - 统一会话与DIAP绑定
            create_unified_session,
            get_unified_session,
            send_alou_message,
            execute_alou_tool,
            list_alou_tool_executors,
            health_check_alou_bridge,
        ])
        ...
        .setup(|app| {
            ...
            // Initialize alou-code Bridge
            let alou_bridge = AlouBridge::new();
            app.manage(Arc::new(alou_bridge));
