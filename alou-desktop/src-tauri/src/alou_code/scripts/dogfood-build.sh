#!/usr/bin/env bash
# dogfood-build.sh — Build alou from current checkout and verify provenance.
#
...
#
#   ALOU=$(bash scripts/dogfood-build.sh)
#
# Then dogfood with config isolation (avoids real user config bleeding in):
#
#   ALOU_CONFIG_HOME=$(mktemp -d) $ALOU plugins list --output-format json
#
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RUST_DIR="$REPO_ROOT/rust"
BINARY="$RUST_DIR/target/debug/alou"
EXPECTED_SHA="$(git -C "$REPO_ROOT" rev-parse --short HEAD)"

echo "▶ Building alou from $REPO_ROOT" >&2
...
echo "" >&2
echo "  export ALOU=$BINARY" >&2
echo "" >&2
echo "  Dogfood with isolated config (no real user config on stderr):" >&2
echo "    ALOU_ISOLATED=\$(mktemp -d)" >&2
echo "    ALOU_CONFIG_HOME=\$ALOU_ISOLATED \$ALOU plugins list --output-format json" >&2
echo "    rm -rf \$ALOU_ISOLATED" >&2
echo "" >&2
echo "  cargo run overhead: ~1s/invocation vs 7ms for pre-built binary." >&2
echo "  Prefer pre-built binary (\$ALOU) for dogfood loops." >&2
echo "$BINARY"
