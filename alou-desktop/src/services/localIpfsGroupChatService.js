...
class LocalIpfsGroupChatService {
  constructor() {
    ...
    this.groups = new Map() // groupId -> LocalGroup (内存存储)
    this.messages = new Map() // groupId -> LocalGroupMessage[] (内存存储)
  ...
  /**
   * 创建本地群聊
   * @param {Object} config - 群聊配置
   * @returns {Promise<LocalGroup>} 创建的群聊信息
   */
   ...
   * @returns {Promise<LocalGroup>} 加入的群聊信息
   */
  ...
  /**
   * 发送消息到群聊
   * @param {LocalGroupMessage|string} message - 消息对象或内容字符串
  ...
  async sendMessage(message, groupId = null, topic = null) {
    ...
    if (!(message instanceof LocalGroupMessage)) {
      throw new Error('消息必须是LocalGroupMessage对象或字符串')
    }
    ...
    try {
      ...
      // 保存到内存
      this.saveMessageToMemory(message.groupId, message)
      
      // 保存到KV存储
      await this.saveMessageToKV(message.groupId, message)

      console.log('[LocalIpfsGroupChatService] 消息发送成功:', message.id)
      return true
   ...
   * 保存消息到内存
   */
  saveMessageToMemory(groupId, message) {
   ...
   * 保存群聊到后端KV存储
   */
  async saveGroupToKV(group) {
    try {
      // 使用后端API存储群聊数据
      const response = await apiClient.post('/kv/set', {
   ...
   * 保存消息到后端KV存储
   */
  async saveMessageToKV(groupId, message) {
    try {
      // 使用后端API存储消息数据
      const response = await apiClient.post('/kv/set', {
   ...
   * 从后端KV存储加载群聊数据
   */
  async loadGroupsFromKV() {
    try {
      // 获取所有群聊相关的键
      const response = await apiClient.get('/kv/keys', {
        params: { prefix: 'group:' }
      })
      ...
      if (response.data.success && response.data.keys) {
        ...
        for (const key of response.data.keys) {
          try {
            const valueResponse = await apiClient.get('/kv/get', {
              params: { key }
            })
   ...
   * 从后端KV存储加载群聊消息
   */
  async loadMessagesFromKV(groupId) {
    try {
      // 获取该群聊的所有消息键
      const response = await apiClient.get('/kv/keys', {
